...
int indexerServer::start( unsigned int type,  unsigned int step_time){
	time_t now;
	tm * local_now;
	char time_buf[128] = {0};
	now = time(NULL);
	time_t now_ago = now - step_time;
	local_now = localtime(&now_ago);
	...
	while(!isstop){
		...
		local_now = localtime(&now_ago);
		now = time(NULL);
		strftime(time_buf, sizeof(time_buf), "%Y-%m-%d %H:%M:%S", local_now);
		
		//check in mysql
		//std::shared_ptr<mysqlconnector> sql_conn = mysql_pool->get_connection();
		//std::ostringstream s_sql;
		s_sql.str("");
		s_sql << "select * from " << type_table << " where rtimestamp>=\"" << time_buf << "\"";
